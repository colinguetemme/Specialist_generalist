...
```{r, eval = FALSE}
geom_mean_fitness = mean(mean_fitness)
``` 
...
```{r, eval = FALSE}
Vmax = 0.5 * sum(loci^2)
geom_mean_fitness = mean(var_gen/Vmax)
``` 
...
* In the model without stochasticity ($d=0$), the time when distance between the distributions seperated by one cycle is under $10^{-12}$, with a maximum of 300 000 generations. 
...
Those values of interest are the **mean fitness** that we are already calculating at every timestep, and the **genetics variance**. Which can be calculated using
