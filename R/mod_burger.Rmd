...
the magnitude of the stochasticity parameter,
the time to stop the simulation.

A good set of values to start could be
...
we then describe the function that will generate 
...
We then can generate the initial probabilities of gametes
...
If we want to transform two gametes considering a list of recombination position we can use a recursive function (or for() loop) to go through all re combination.
...
And the last function we will need is a function that does this for every combination of j, k and i. So it will basically creates a big dataframe and add the probability to have the gamete i considering a pair j and k of gametes, which corresponds to the 3 for() loops.
