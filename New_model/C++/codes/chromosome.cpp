...
void chromosome::add_mutation(para_ind para) {
	...
	if (mutations.find(position) == mutations.end()) { // No mutation at that position DO WE NEED IT //!//
		...
		double deleterious = runif(gen);
		if(deleterious < para.prop_del){
			deleterious = -1;
		} else {
			deleterious = 1;
		}
		// The value of the mutation is sampled from the corresponding normal distribution
		mut.value = deleterious * para.distr_mut_value(gen);
		...
		if (para.trade_off == 1){
			// Check if the trade-off apply to this mutation (beneficial)
			// or with the trade_off_del == 1
			if ((para.trade_off_del == 1) || (para.trade_off_del == 0 && mut.value > 0)){
				...
				if (mutations.find(position_t_o) == mutations.end()) {
					...
					mut_t_o.value = -mut.value * para.trade_off_strength; //!// should be a parameter and maybe different if positive and negative
 ...
 * @param para the parameters of the individuals
 */
void chromosome::add_mutation2(para_ind para) {
	...
	if (mutations.find(position) == mutations.end()) { // No mutation at that position DO WE NEED IT //!//
		...
		// Start by sampling two values from the distribution and then look at which one is the min and max
		double value = runif(gen);
		double range = runif(gen);
		bool direction = round(runif(gen));
		if (direction == 0){
			mut.min = value - range;
			mut.max = value;
		} else {
